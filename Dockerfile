...
RUN \
apt-get update && \
DEBIAN_FRONTEND=noninteractive apt-get install -q -y supervisor xvfb x11vnc xrdp ibus-mozc mozc-utils-gui fonts-ipafont fonts-ricty-diminished vim less ubuntu-mate-desktop && \
DEBIAN_FRONTEND=noninteractive apt-get remove -q -y blueman geoclue-2.0 && \
